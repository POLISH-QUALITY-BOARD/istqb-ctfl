...
## Profesijné ciele {.business-outcomes}
...
1. Chápať, čo je testovanie a prečo je prospešné. |
1. Chápať základné koncepty testovania softvéru. |
1. Určiť prístup k testovaniu a činnosti, ktoré majú byť vykonané v závislosti od kontextu. |
1. Posúdiť a zlepšiť kvalitu dokumentácie. |
1. Zvýšiť efektivitu a účinnosť testovania. |
1. Zladiť proces testovania so životným cyklom vývoja softvéru. |
1. Rozumieť princípom manažmentu testovania. |
1. Písať jasné a zrozumiteľné reporty o defekte a vedieť ich komunikovať. |
1. Chápať faktory, ktoré ovplyvňujú priority a náročnosť činností súvisiacich s testovaním. |
1. Pracovať ako súčasť multifunkčného tímu. |
1. Rozumieť rizikám a prínosom automatizácie testov. |
1. Identifikovať nevyhnutné zručnosti potrebné na testovanie. |
1. Chápať vplyv rizík na testovanie. |
1. Efektívne reportovať o postupe a kvalite testovania. |
...
Na začiatku každej kapitoly (tesne pod názvom kapitoly) je uvedený zoznam kľúčových slov. Všetky uvedené kľúčové slová je nutné si zapamätať (K1), a to aj v prípade, že nie sú výslovne uvedené v študijných cieľoch. Viac informácií o klasifikácii úrovní znalostí vrátane príkladov možno nájsť v [sekcii <#section:cognitive-levels>{-}](#section:cognitive-levels).

## Certifikačná skúška základnej úrovne

Skúška pre získanie certifikátu z modulu Certifikovaný Tester Základná Úroveň je založená na týchto učebných osnovách. Odpovede na jednotlivé otázky pri certifikačnej skúške môžu vyžadovať znalosti z rôznych kapitol. Všetky časti týchto osnov môžu byť predmetom certifikačnej skúšky (okrem úvodu a príloh). Normy, knihy a iné osnovy ISTQB® sú uvedené ako odkazy (pozri <#section:references>), ale ich obsah nie je súčasťou skúšky s výnimkou toho, čo je z nich priamo zahrnuté v týchto osnovách. Viac informácií je možné nájsť v dokumente Štruktúry a pravidlá skúšok základnej úrovne 
...
V týchto učebných osnovách sú uvedené odkazy na rôzne normy (napr. normy IEEE alebo ISO). Informácie v týchto normách môžu byť citované priamo (napr. norma ISO 25010 pri definícii kvalitatívnych charakteristík) alebo poskytujú prípadnému čitateľovi zdroj ďalších informácií. Samotné normy však nie sú predmetom skúšky. Viac informácií možno nájsť v <#section:references>.
