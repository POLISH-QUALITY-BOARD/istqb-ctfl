# Testovacia analýza a návrh testov – 390 minút {#test-analysis-and-design}
...
## Techniky testovania čiernej skrinky {#black-box-test-techniques}
...
## Techniky testovania bielej skrinky {#white-box-test-techniques}
...
## Techniky testovania založené na skúsenostiach {#experience-based-test-techniques}
...
Prieskumné testovanie je najviac užitočné v prípadoch, keď je špecifikácia nedostatočná alebo úplne chýba, taktiež je vhodnou metódou v prípade výrazného časového tlaku na testovanie. Je tiež užitočné ako doplnok iných (formálnejších) testovacích techník. Všeobecne bude efektívnejšie v prípadoch, keď sú testeri skúsení, majú znalosti z danej domény a vysokú úroveň základných zručností ako sú analytické schopnosti, zvedavosť a kreativita (pozri <#section:general-skills-required-for-testing>).
...
## Prístupy k testovaniu založené na spolupráci {#collaboration-based-test-approaches}

Každá z vyššie uvedených techník (pozri <#section:black-box-test-techniques>, <#section:white-box-test-techniques>, <#section:experience-based-test-techniques>) má svoj špecifický cieľ vo vzťahu k identifikácii defektov. Na druhej strane prístupy založené na spolupráci sa zameriavajú aj na predchádzanie vzniku chýb prostredníctvom spolupráce a komunikácie.

### Spoločné písanie používateľských scenárov {#collaborative-user-story-writing}
...
* orientovaný na scenáre (napr. formát Given/When/Then používaný v BDD, pozri <#section:testing-as-driver-for-sw-development>),
...
ATDD je jeden z prístupov iniciovaných testami (test-first) (pozri <#section:testing-as-driver-for-sw-development>), kedy sú testovacie prípady vytvorené pred vlastnou implementáciou používateľského scenára členmi tímu s rôznymi perspektívami pohľadu, ako napr. zákazníci, vývojári a testeri [@adzic2009bridging]. Testovacie prípady sa môžu vykonávať manuálne alebo automatizovane.
