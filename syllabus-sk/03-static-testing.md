# Statické testovanie – 80 minút {#static-testing}
...
Statické a dynamické testovacie postupy sa vzájomne dopĺňajú. Majú podobné ciele (napr. odhaľovanie defektov v pracovných produktoch, pozri <#section:test-objectives>), ale existujú medzi nimi aj určité rozdiely, napríklad:
...
* Statické testovanie je možné použiť na meranie kvalitatívnych charakteristík (napr. udržteľnosti) nezávislých na spustení kódu, dynamické testovanie iba na tie, ktoré sú na spustení kódu závislé (napr. výkonnostná efektivita).
...
### Prínosy včasnej a častej spätnej väzby od zainteresovaných strán {#benefits-of-early-and-frequent-feedback}
...
* **Opravy a reportovanie**. Aby bolo možné vykonať nápravu, mal by byť pre každý defekt vytvorený report o defekte. Pri splnení daných výstupných kritérií môže byť pracovný produkt akceptovaný a výsledky revízie sú podané v správe.
...
* **Moderátor** (niekedy aj facilitátor) – zaisťuje efektívny priebeh revíznych schôdzok vrátane využitia mediácie. Okrem iného dohliada na dodržanie časového rámca a vytvorenie bezpečného prostredia, v ktorom môže každý slobodne vyjadriť svoj názor.
...
* Zainteresovaným stranám a autorom je poskytovaná spätná väzba z revízií tak, aby mohli zlepšovať produkt a svoje činnosti (pozri <#section:benefits-of-early-and-frequent-feedback>).
